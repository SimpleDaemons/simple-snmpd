...
:build_project
call :print_status "Building simple-snmpd..."
...
:install_project
if "%INSTALL%"=="true" (
    call :print_status "Installing simple-snmpd..."
    cd /d "%BUILD_DIR%"
...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-snmpd-0.1.0-static-windows"
    if not exist "%STATIC_DIR%" mkdir "%STATIC_DIR%"
    
    REM Copy binary and files
    copy "%BUILD_DIR%\simple-snmpd.exe" "%STATIC_DIR%\"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-snmpd-0.1.0-static-windows' -DestinationPath 'simple-snmpd-0.1.0-static-windows.zip' -Force"
    rmdir /s /q "simple-snmpd-0.1.0-static-windows"
    
    call :print_success "Static binary package created: simple-snmpd-0.1.0-static-windows.zip"
)
goto :eof

REM Function to show usage
:show_usage
echo simple-snmpd - Windows Build Script
echo.
...
REM Main execution
:main
call :print_status "Starting simple-snmpd build process..."
...
call :print_success "simple-snmpd build process completed successfully!"
goto :end
