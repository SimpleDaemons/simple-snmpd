#!/bin/bash
# Generic Linux build script for simple-snmpd
...
build_project() {
    print_status "Building simple-snmpd..."
...
install_project() {
    if [ "$INSTALL" = "true" ]; then
        print_status "Installing simple-snmpd..."
        cd "$BUILD_DIR"
        
        sudo make install
        
        # Test installation
        if command_exists simple-snmpd; then
...
create_service() {
    if [ "$SERVICE" = "true" ]; then
        ...
        sudo tee /etc/systemd/system/simple-snmpd.service > /dev/null << EOF
[Unit]
Description=Simple SNMP Daemon - A lightweight and secure SNMP monitoring daemon
...
User=simple-snmpd
Group=simple-snmpd
ExecStart=/usr/local/bin/simple-snmpd --daemon
Restart=always
RestartSec=5
...
EOF
        ...
        sudo useradd --system --no-create-home --shell /bin/false simple-snmpd 2>/dev/null || true
        ...
        print_status "Use 'sudo systemctl start simple-snmpd' to start the service"
    fi
}
...
show_usage() {
    cat << EOF
simple-snmpd - Linux Build Script
...
# Main execution
main() {
    print_status "Starting simple-snmpd build process..."
    ...
    print_success "simple-snmpd build process completed successfully!"
}
