...
# simple-snmpd macOS Build Script
# This script builds the simple-snmpd application for macOS
...
create_static_package() {
    if [ "$ENABLE_STATIC_LINKING" = "ON" ]; then
        ...
        STATIC_DIR="$DIST_DIR/simple-snmpd-$VERSION-static-macos"
        mkdir -p "$STATIC_DIR"
        
        # Copy binary and files
        cp "$BUILD_DIR/simple-snmpd" "$STATIC_DIR/"
        ...
        # Create tarball
        cd "$DIST_DIR"
        tar -czf "simple-snmpd-$VERSION-static-macos.tar.gz" "simple-snmpd-$VERSION-static-macos"
        rm -rf "simple-snmpd-$VERSION-static-macos"
        
        print_success "Static binary package created: simple-snmpd-$VERSION-static-macos.tar.gz"
    fi
}
